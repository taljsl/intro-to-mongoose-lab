...
const updateProfile = async () => {
    ...
    const findProfile = prompt('Please paste in the id of the person you wish to update here')
    if(!findProfile){
    ...
    if(whatToChange == '1'){
        ...
        let newAge = prompt('What would you like to make the age for this profile?: ')
        const updatedProfile = await Profiles.findByIdAndUpdate(
            findProfile,
            {age: newAge}
        )
...
const deleteProfile = async () => {
    ...
    const safety = prompt ('We found that profile', findProfile, '\n Are you sure you want to delete it? Press y/n: ')
    if (safety.toLocaleLowerCase() == 'y' ){
        const deletedProfile = await Profiles.findByIdAndDelete(findProfile);
        console.log('Profile Deleted')
          } else if (safety.toLowerCase()){
